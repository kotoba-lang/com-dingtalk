(ns dingtalk.main
  "Kotodama WASM entrypoint for the Dingtalk clean-room actor (L4) — Clojure port.
  ...
  facts (`dingtalk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dingtalk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dingtalk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dingtalk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dingtalk-compat/ready)
